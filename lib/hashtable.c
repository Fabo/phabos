...
static const size_t HASHTABLE_MIN_SIZE = 2;
                                                 ...
                                                 bool find_deleted)
{
    ...
    while ((ht->table[h].is_used && ht->table[h].key != key) ||
...
static void hashtable_resize(hashtable_t *ht, size_t size)
{
    ...
    memset(ht->table, 0, sizeof(size * sizeof(struct hashtable_node)));
    ht->size = size;
    ht->count = 0;
...
void hashtable_init(hashtable_t *ht, hashtable_hash_fct_t hash)
{
    ...
    memset(ht->table, 0, sizeof(HASHTABLE_MIN_SIZE * sizeof(struct hashtable_node)));
    ht->hash = hash;
